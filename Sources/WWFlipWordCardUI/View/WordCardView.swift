        ...
        Group {
            if !isFlipped {
                ...
                ZStack(alignment: .topLeading) {
                    ...
                    categoryBadge(wordCard.categories)
                        .padding(.top, 12)
                        .padding(.leading, 12)
                    
                    if !wordCard.level.isEmpty {
...
private extension WordCardView {
    ...
    var frontView: some View {
        ...
        ZStack(alignment: .bottomTrailing) {
            ...
            let wordView = Text(wordCard.word)
                .foregroundStyle(.black)
                .multilineTextAlignment(.center)
                .lineLimit(1)
                .minimumScaleFactor(0.5)
                .font(configure.wordFont)
            
            let readingView = Text(wordCard.reading)
                .foregroundStyle(.orange)
                .multilineTextAlignment(.center)
                .lineLimit(1)
                .minimumScaleFactor(0.6)
                .font(configure.readingFont)
    ...
    func levelBadge(_ level: String) -> some View {
        ...
        let color = configure.levelColors[level] ?? .blue
...
private extension WordCardView {
    ...
    /// - Returns: View
    func categoryBadge(_ categorties: [String]) -> some View {
                
        return HStack(spacing: 4) {
            
            ForEach(categorties, id: \.self) { categorty in
            
                let color = configure.categoryColors[categorty] ?? .blue
